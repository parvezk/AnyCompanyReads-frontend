...
const checkValidation = (value, validation) => {
  if (validation.numValues?.length) {
    switch (validation.type) {
      ...
      case "LessThanEqualToNum": 
        return {
          hasError: !(value <= validation.numValues[0]),
          errorMessage:
            validation.validationMessage ||
            `The value must be less than or equal to ${validation.numValues[0]}`,
        };
      case "GreaterThanNum":
        return {
          hasError: !(value > validation.numValues[0]),
      ...
      case "BetweenInclusive":
        return {
          hasError: !(value >= validation.numValues[0] && value <= validation.numValues[1]),
          errorMessage:
            validation.validationMessage ||
            `The value must be between ${validation.numValues.join(" and ")}`,
        };
      default:
    }
  ...
  switch (validation.type) {
    ...
    case "URL":
      ...
      return {
        ...
        errorMessage:
          validation.validationMessage ||
          "The value must be a valid URL that begins with a schema (i.e. http://)",
      };
    case "Phone":
      const PHONE = /^\+?\d[\d\s-]+$/;
      return {
